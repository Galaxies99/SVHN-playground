...
class BetaVAE(nn.Module):
    ...
    def sample_feature(self, feature_dim, n_images, n_changing, device, **kwargs):
        z = torch.randn(n_images, 1, self.latent_dim).to(device)
        step = 10.0 / n_changing
        z = z.repeat(1, n_changing, 1)
        for i in range(n_images):
            for j in range(n_changing):
                z[i, j, feature_dim] = -5.0 + step * j
        z = rearrange(z, 'n c d -> (n c) d')
        return self.decode(z)
