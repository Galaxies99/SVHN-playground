...
class DFCVAE(nn.Module):
    ...
    def extract_feature(self, x, feature_layers = ['0', '5', '10']):
        features = []
    ...
    def loss(self, recon, x, mu, log_var, x_feature, recon_feature, **kwargs):
        ...
        feature_loss = F.mse_loss(recon, x)
        for (recon_f, x_f) in zip(recon_feature, x_feature):
            feature_loss += F.mse_loss(recon_f, x_f)
        feature_loss = feature_loss / (len(x_feature) + 1)
        kl_weight = kwargs['kl_weight']
        loss = self.alpha * kl_loss * kl_weight + self.beta * feature_loss
        return {
            'loss': loss,
